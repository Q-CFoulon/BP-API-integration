...
exports.socAlertWorkflow = socAlertWorkflow;
...
async function socAlertWorkflow() {
    console.log('═══════════════════════════════════════════════════════════');
    ...
    const lifecycleService = (0, lifecycle_service_1.createLifecycleService)();
    try {
        ...
        if (criticalAlerts.length > 0) {
            ...
            lifecycleService.logAction(alertId, 'acknowledged', 'Alert acknowledged in SOC dashboard', 'john.doe@company.com');
            ...
            lifecycleService.addNotes(alertId, 'correlates with user working from temporary location. Checking against known IPs.', 'sarah.smith@company.com');
            console.log(`   ✓ Investigation notes added\n`);
            // Action 5: Additional context
            lifecycleService.logAction(alertId, 'verified_context', 'Connected with client - confirmed legitimate remote access', 'sarah.smith@company.com', { clientConfirmed: true, reason: 'WFH approval on file' });
...
//# sourceMappingURL=soc-workflow-complete.js.map