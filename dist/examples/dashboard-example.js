...
async function runDashboard() {
    try {
        ...
        summary.tenantAlerts.slice(0, 10).forEach((tenant, index) => {
            console.log(`${index + 1}. ${tenant.tenantName.padEnd(30)} ` +
...
async function getClientDetails(clientName) {
    try {
        ...
        details.alerts.forEach((alert) => {
            console.log(`  [${alert.severity}] ${alert.status.toUpperCase()}: ${alert.title} ` +
 ...
 */
async function monitorAlerts(intervalSeconds = 300) {
    try {
        const apiClient = (0, blackpoint_api_service_1.createApiClient)();
        const dashboardService = (0, dashboard_service_1.createDashboardService)(apiClient);
        console.log(`🔍 Starting alert monitor (refreshing every ${intervalSeconds} seconds)...\n`);
        setInterval(async () => {
            try {
                const summary = await dashboardService.getDashboardSummary();
                const criticalAlerts = await dashboardService.getCriticalAlerts();
...
//# sourceMappingURL=dashboard-example.js.map